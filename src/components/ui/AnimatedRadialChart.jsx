...
// Helper function to merge class names (fallback if clsx not available)
const cn = (...classes) => {
  return clsx(...classes);
};
...
export function AnimatedRadialChart({ 
  ...
  // Get animated percentage value for display
    ...
    <div className={cn("relative", className)} style={{ width: size, height: size * 0.7 }}>
      <svg width={size} height={size * 0.7} viewBox={`0 0 ${size} ${size * 0.7}`} className="overflow-visible">
        ...
        <motion.line
          ...
          x2={useTransform(progressAngle, (angle) => {
            const cos = Math.cos(angle);
            return center + cos * innerRadius - cos * 30;
          })}
          y2={useTransform(progressAngle, (angle) => {
            const sin = Math.sin(angle);
            return center + sin * innerRadius - sin * 30;
          })}
