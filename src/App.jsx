...
function AppContent() {
  ...
  // Initialize auth checking based on token presence to prevent FOUC
  ...
  useEffect(() => {
    const checkAuthStatus = async () => {
      try {
        ...
        if (token) {
          // We are checking auth, so keep isAuthChecking true (it was initialized to true if token exists)
        ...
        // If we were checking auth, we are done now
        // We don't set isAuthChecking(false) here immediately if we want to show the loader
        // Instead, we rely on the CounterLoader's onComplete or set it here if no loader is shown
        // But since we want to show the loader if isAuthChecking is true, we let the loader handle the transition
        // However, if the check failed (no user), we should stop checking immediately to show landing page
        const token = localStorage.getItem('authToken');
        if (!token) {
          setIsAuthChecking(false);
        }
        // If token exists and valid, we keep isAuthChecking true until CounterLoader finishes
      }
    };
  ...
  useEffect(() => {
    if (isAuthenticated && currentUser) {
      ...
      // Check every 5 minutes - optimized to prevent refresh issues
    ...
    if (typeof window !== 'undefined') {
      ...
      const monitorToolbar = setInterval(() => {
        try {
          ...
          console.error('Toolbar monitoring error:', error);
          clearInterval(monitorToolbar);
        }
      }, checkInterval);
  ...
  return (
    <GradientBackground>
      <div className="relative min-h-screen text-white">
        ...
        />
        {isAuthChecking ? (
          <CounterLoader onComplete={handleAuthCheckComplete} duration={1500} />
        ) : isDashboardLoading ? (
